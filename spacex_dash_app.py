...
def get_pie_chart(entered_site):
    ...
    filtered_df['Outcome'] =np.where(filtered_df['class']==1, 'Success', 'Failure')
    filtered_df =filtered_df.rename(columns = {'Launch Site':'Count'})
    if entered_site == 'ALL': 
...
def get_scatter_chart(entered_site, payload):
    ...
    #print(payload[1])
    df = spacex_df
    df = df.drop(df[df['Payload Mass (kg)']<payload[0]].index)
    df = df.drop(df[df['Payload Mass (kg)'] >payload[1]].index)
